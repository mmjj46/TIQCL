...
rep_number = 3000  # [수정] 1회성 pulse는 오실로스코프에서 캡처하기 어려움.
                    # 300us(freq_scan gate time) x 3000회 = 약 0.9초 동안 반복 재생되어
                    # 오실로스코프가 안정적으로 파형을 잡을 수 있게 함. (0: infinite loop)
...
def time_scan(jj, samplerate):
	...
	# Phase (Rad)
	# [수정] time scan에서는 phase를 반드시 0으로 고정해야 함.
	# (이전 코드에서는 phase_single_rad = jj * 0.1 * np.pi 로 되어있어
	#  실제로는 phase scan이 되어버리는 버그가 있었음)
...
def freq_scan(jj, samplerate):
	"""Monotonic sine wave generator
	(carrier 주변을 넓게 스캔해서 CAR/RSB/BSB 봉우리 위치를 찾는 용도)"""

	# Frequency (MHz) : 대략적인 carrier 위치 (motional freq는 통상 수백 kHz~수 MHz 대역)
	freq_CAR_MHz = 71.822

	# 전체 스캔 범위 (MHz). CAR을 중심으로 좌우로 이만큼 훑음.
	# 노트에 그린 것처럼 RSB(a) - CAR(dashed, b 근처) - BSB(c)가 다 보이도록
	# motional freq보다 넉넉하게 잡아야 함. 필요시 조정.
	scan_range_MHz = 4.0

	start_freq_MHz = freq_CAR_MHz - scan_range_MHz / 2

	# step_number(전역 변수)로 전체 범위를 나눠서 step 크기를 자동 계산
	if step_number > 1:
		step_freq_MHz = scan_range_MHz / (step_number - 1)
	else:
		step_freq_MHz = 0.0

	freq_scan_MHz = start_freq_MHz + jj * step_freq_MHz

	# Phase (Rad)
	phase_single_rad = 0.0

	# Amplitude
	amp_single = 0.05 #0.04 # 0.6  # 0 ≤ amp_single ≤ 1
...
# [추가] freq_scan을 쓰는 경우, 각 step의 주파수를 미리 계산해서 콘솔 출력용으로 저장
# (오실로스코프로 확인할 때 "지금 몇 MHz인지" 바로 알 수 있게)
freq_MHz_list = None
if calculate_func == freq_scan:
	_freq_CAR_MHz = 71.822
	_scan_range_MHz = 4.0
	_start_freq_MHz = _freq_CAR_MHz - _scan_range_MHz / 2
	if step_number > 1:
		_step_freq_MHz = _scan_range_MHz / (step_number - 1)
	else:
		_step_freq_MHz = 0.0
	freq_MHz_list = [_start_freq_MHz + jj * _step_freq_MHz for jj in range(step_number)]
...
for jj in range(step_number):
	...
	if freq_MHz_list is not None:
		print(f"  → 이번 step 주파수: {freq_MHz_list[jj]:.4f} MHz  (오실로스코프에서 확인해보세요)")
	...
	# Software trigger 사용 시(trig_number == 2), 강제로 트리거 발생시켜야 카드가 진행됨.
	# 이게 없으면 M2CMD_CARD_WAITREADY에서 timeout(현재 100초)까지 멈춰있음.
	if trig_number == 2:
	...
	# [추가] 오실로스코프로 신호를 확인할 시간을 주기 위한 일시정지
	# Enter를 누르면 다음 step으로 넘어감
	input("  >> 오실로스코프에서 신호 확인 후 Enter를 누르면 다음 step으로 넘어갑니다 ...")
...
if SHOW_WAVEFORMS == True:
	...
	plt.plot(buffers[jj_show][:5000], label=f"jj = {jj_show}")
	plt.title('')
